...
```{r}
#after getting the associated functional categories, some genes
#did not have an available category, so I check once again that
#each TF has >20 targets
...
```{r}
#raw weights
raw_weights <- table(gene_func_cat$FunctionCategory)/length(gene_func_cat$FunctionCategory)
#normalized
...
for (tf in reg_net_final){
  ...
  entropy=entropy*-1
  entropies[[i]]=list(TF=tf$TF, entropy=entropy)
  i=i+1
}
```
